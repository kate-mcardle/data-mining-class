...
hf = hf[which(hf$proc_p==3734 & hf$race==1),]
...
# Charges
nB.charges = naiveBayes(charge ~ agecat5+adm_src+pay_cat+dnr+oprocs+odiags+size, data = charges.train, laplace = 1)
...
# LOS -----------------------------
nB.los = naiveBayes(los ~ agecat5++adm_src+pay_cat+dnr+oprocs+odiags+size, data = los.train, laplace = 1)
# Make class predictions
