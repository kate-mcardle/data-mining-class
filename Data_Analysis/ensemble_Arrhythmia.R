...
# Filter out records where proc_p != 3734
# hf = hf[,c("proc_p","los","charge","agecat5","sex","race","patcnty","adm_src","pay_cat","dnr","oprocs","odiags","oshpd_id")]
hf = hf[,c("proc_p","los","charge","agecat5","adm_src","pay_cat","dnr","oprocs","odiags","oshpd_id")]

hf$proc_p = as.integer(hf$proc_p)
hf = hf[which(hf$proc_p==3734),]
...
lassoCHG = logRegCV.charges$lambda.min
# s=fit1cv$lambda.min
...
lassoLOS = logRegCV.los$lambda.min
# s=fit1cv$lambda.min
# Run logistic regression model on test data
...
conf.LosGBM = table(los.test$los, predictedLos.GBM)
...
par(opar)
par(lty=4, cex=1.8)
plot(perfCharges.NB, main='ROC curve for Charges - Cardiac Arrhythmia', xlab = "False Positive Rate", ylab="True Positive Rate", add = FALSE)
par(lty=2, col="red", pch=18)
...
par(opar)
par(lty=4, cex=1.8)
plot(perfChargesLift.NB, main='Lift curve for Charges - Cardiac Arrhythmia', xlab="Rate of Positive Predictions", ylab="Lift", add = FALSE)
par(lty=2, col="red")
...
par(opar)
par(lty=4, cex=1.8)
plot(perfLos.NB, main='ROC curve for LOS - Cardiac Arrhythmia', xlab = "False Positive Rate", ylab="True Positive Rate", add = FALSE)
par(lty=2, col="red")
...
par(opar)
par(lty=4, cex=1.8)
plot(perfLosLift.NB, main='Lift curve for LOS - Cardiac Arrhythmia', xlab="Rate of Positive Predictions", ylab="Lift", add = FALSE)
par(lty=2, col="red")
