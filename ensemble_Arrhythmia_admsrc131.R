...
hf = hf[which(hf$proc_p==3734 & hf$adm_src==131),]
...
# Charges
nB.charges = naiveBayes(charge ~ agecat5+pay_cat+dnr+oprocs+odiags+size+agerace, data = charges.train, laplace = 1)
...
# 
# # LOS -----------------------------
nB.los = naiveBayes(los ~ agecat5+pay_cat+dnr+oprocs+odiags+size+agerace, data = los.train, laplace = 1)
# Make class predictions
