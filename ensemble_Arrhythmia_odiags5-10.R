...
# Keep only records where proc_p = 3734 and odiags between 6-10
# hf = hf[,c("proc_p","los","charge","agecat5","sex","race","patcnty","adm_src","pay_cat","dnr","oprocs","odiags","oshpd_id")]
hf = hf[,c("proc_p","los","charge","agecat5","race","adm_src","pay_cat","dnr","oprocs","odiags","oshpd_id")]
hf$proc_p = as.integer(hf$proc_p)
hf = hf[which(hf$proc_p==3734 & hf$odiags<11),]
...
# Charges
nB.charges = naiveBayes(charge ~ agecat5+adm_src+pay_cat+dnr+oprocs+odiags+size+agerace, data = charges.train, laplace = 1)
...
# LOS -----------------------------
nB.los = naiveBayes(los ~ agecat5++adm_src+pay_cat+dnr+oprocs+odiags+size+agerace, data = los.train, laplace = 1)
# Make class predictions
...
x.charges = model.matrix(~-1+agecat5+adm_src+pay_cat+dnr+agerace, data=charges.train)
x.charges = cbind(x.charges, charges.train$oprocs, charges.train$odiags, charges.train$size)
y.charges = charges.train$charge
testx.charges = model.matrix(~-1+agecat5+adm_src+pay_cat+dnr+agerace, data=charges.test)
...
# LOS ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
# Convert training and test data to model matrices
x.los = model.matrix(~-1+agecat5+adm_src+pay_cat+dnr+agerace, data=los.train)
x.los = cbind(x.los, los.train$oprocs, los.train$odiags, los.train$size)
y.los = los.train$los
testx.los = model.matrix(~-1+agecat5+adm_src+pay_cat+dnr+agerace, data=los.test)
